...
import { listStanden, resolveOplossingTekst, bulkAddCategorieWaarde } from "../db/standen.js?v=20260924f";
...
export async function renderDatabaseView(container, { onOpenStand, onAddSelectionToStencil } = {}) {
  ...
  el('[data-action="share-selection"]').addEventListener("click", async () => {
    const { buildShareData } = await import("../db/backup.js?v=20260924f");
    const { encodeShareData } = await import("../db/shareLink.js?v=20260924f");
